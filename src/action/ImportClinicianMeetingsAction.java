...
/**
 * The Class ImportClinicianMeetingsAction handles the importing of a meeting
 * schedule from an excel file.
 * 
...
public class ImportClinicianMeetingsAction {
	...
	/**
	 * Insert imported meetings into the commitments table for each clinician
	 * specified for a meeting.
	 *
	 * @param endDate
	 *            the end date
	 * @throws SQLException
	 ...
	 *             the invalid excel format exception
	 */
	public void insertImportedMeetings(Date endDate) throws SQLException,
		...
		try {
			...
			if (rowIterator.hasNext()) {
				...
				throw new InvalidExcelFormatException(
						"Excel document contains no data");
			}
	...
	/**
	 * Parses the excel row provided and inserts meeting corresponding to the
	 * meeting details specified in this row into the database.
	 *
	 * @param excelRow
	 *            the excel row
	 * @param endDate
	 ...
	 *             the SQL exception
	 */
	private void parseRowAndInsertMeetings(Map<String, Object> excelRow,
			Date endDate) throws InvalidExcelFormatException, SQLException {
	...
	/**
	 * Inserts meetings for the provided dates and staffMembers at the provided
	 * time with the given meeting name into the database.
	 *
	 * @param meetingDates
	 *            the meeting dates
	 * @param staffMembers
	 ...
	 *             the SQL exception
	 */
	private void insertMeetings(List<Date> meetingDates, String staffMembers,
			int sTime, int eTime, String meetingName) throws SQLException {
		...
		if (staffMembers.equalsIgnoreCase("ALL")) {
			...
			for (Date meetingDate : meetingDates) {
				for (ClinicianBean clinician : clinicians) {
					...
					CommitmentBean commitment = new CommitmentBean(clinicianID,
							sTime, eTime, meetingDate, meetingName);
			...
			for (Date meetingDate : meetingDates) {
				for (String staffMember : staff) {
					...
					if (clinicianID != -1) {
						CommitmentBean commitment = new CommitmentBean(
								clinicianID, sTime, eTime, meetingDate,
								meetingName);
						commitmentsDAO.insert(commitment);
					}
				}
	...
	/**
	 * Parses the time in the specified string, rounding up to the nearest hour
	 * if specified.
	 *
	 * @param timeString
	 *            the time string to parse
	 * @param roundUp
	 *            specify whether it should the round up to the next hour
	...
	private int parseTime(String timeString, boolean roundUp) {
		...
		if (colonIndex != -1 && timeString.length() > colonIndex + 3) {
			if (!roundUp || timeString.contains(":00")) {
				timeString = timeString.substring(0, colonIndex) + ":00 "
						+ timeString.substring(colonIndex + 3);
			} else {
	 ...
	 * @return the string
	 */
	private String formatInvalidExcelFormatString(String cellData,
			String cellTitle) {
		if (cellData == null || cellData.isEmpty()) {
			return "Data under column '" + cellTitle
					+ "' is of an invalid format 'Entry was not specified'";
		} else {
			return "Data under column '" + cellTitle
					+ "' is of an invalid format '" + cellData + "'";
		}
	}

	/**
	 * Gets the meeting dates when they are scheduled weekly (or biweekly) on a
	 ...
	 * @param endDate
	 *            the end date
	 * @param daysBetweenMeetings
	 *            the days between meetings
	 * @param day
	 *            the day of the week
	 * @return the meeting dates at the specified frequency from start date to
	 *         end date on the day specified
	 */
	public static List<Date> getMeetingDatesWeekly(Date startDate,
			Date endDate, int daysBetweenMeetings, String day) {
	...
	/**
	 * Gets the meeting dates when they are scheduled monthly (e.g. 1st tuesday
	 * of month).
	 *
	 * @param startDate
	 *            the start date
	 ...
	 *            the end date
	 * @param dayOfMonth
	 *            the day of month (e.g. 2nd tuesday)
	 * @param day
	 *            the day of the week
	 * @return the meeting dates monthly for the specified day of the week
	 */
	public static List<Date> getMeetingDatesMonthly(Date startDate,
			Date endDate, int dayOfMonth, String day) {
		List<Date> dates = new ArrayList<Date>();
		Calendar calStart = Calendar.getInstance();
		calStart.setTime(startDate);
		Calendar calEnd = Calendar.getInstance();
		...
		int dayOfWeek = Weekday.valueOf(day).toCalendarWeekday();
		calStart.set(Calendar.DAY_OF_MONTH, Calendar.getInstance()
	...
	/**
	 * Parses the specified meeting dates which have been specified explicitly
	 ...
	 *             the invalid excel format exception
	 */
	private List<Date> parseSpecifiedMeetingDates(Date startDate, Date endDate,
			String dates) throws InvalidExcelFormatException {
		...
		for (String dateSpecified : datesSpecified) {
			try {
				...
				if (cal.getTime().after(endDate)
						|| cal.getTime().before(startDate)) {
					throw new InvalidExcelFormatException(
							formatInvalidExcelFormatString(dates, "Dates"));
				} else {
	...
	/**
	 * Parses the monthly specifier (e.g. 1st Tuesday) and returns it as an
	 ...
	 *             the invalid excel format exception
	 */
	private int parseMonthlySpecifier(String daySpecified)
			throws InvalidExcelFormatException {
		...
		if (firstIndex != -1
				&& (secondIndex != -1 || thirdIndex != -1 || fourthIndex != -1)) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(daySpecified, "Days"));
		} else if (secondIndex != -1
				&& (firstIndex != -1 || thirdIndex != -1 || fourthIndex != -1)) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(daySpecified, "Days"));
		} else if (thirdIndex != -1
				&& (firstIndex != -1 || secondIndex != -1 || fourthIndex != -1)) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(daySpecified, "Days"));
	 ...
	 *             the invalid excel format exception
	 */
	private List<Date> parseDays(Date startDate, Date endDate, String days,
			String frequency) throws InvalidExcelFormatException {
		...
		for (String daySpecified : daysSpecified) {
			...
			if (!Weekday.contains(daySpecified)) {
				...
				if (daySpecified.length() <= 3) {
					...
					if (!(day.equals(Weekday.Monday.name())
							|| day.equals(Weekday.Tuesday.name())
							|| day.equals(Weekday.Wednesday.name())
							|| day.equals(Weekday.Thursday.name()) || day
								.equals(Weekday.Friday.name()))) {
						throw new InvalidExcelFormatException(
								formatInvalidExcelFormatString(daySpecified,
										"Days"));
					} else {
						if (!frequency.equals("Monthly")) {
							throw new InvalidExcelFormatException(
									formatInvalidExcelFormatString(frequency
											+ " and " + daySpecified,
											"Frequency and Days"));
						}
						List<Date> monthlyDates = getMeetingDatesMonthly(
								startDate, endDate, dayOfMonth, day);
				...
				if (frequency.equals("Monthly")) {
					throw new InvalidExcelFormatException(
							formatInvalidExcelFormatString(frequency + " and "
									+ daySpecified, "Frequency and Days"));
				}
				List<Date> weeklyDates = getMeetingDatesWeekly(startDate,
						endDate, daysBetweenMeetings, daySpecified);
	...
	/**
	 * Gets the meeting dates from a particular meeting specified in a row of
	 * the excel document.
	 *
	 * @param excelRow
	 *            the excel row
	 ...
	 *             the invalid excel format exception
	 */
	private List<Date> getMeetingDates(Map<String, Object> excelRow,
			Date endDate) throws InvalidExcelFormatException {

		Date startDate = (Date) excelRow.get("Start Date");
		if (startDate == null) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(null, "Start Date"));
		}
		...
		if (frequency == null
				|| frequency.isEmpty()
				|| !(frequency.equals("Weekly") || frequency.equals("Biweekly") || frequency
						.equals("Monthly"))) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(frequency, "Frequency"));
		}

		String days = (String) excelRow.get("Days");
		if (days == null || days.isEmpty()) {
			throw new InvalidExcelFormatException(
					formatInvalidExcelFormatString(days, "Days"));
		}
