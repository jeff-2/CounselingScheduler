...
public class ClinicianLoadPreferencesAction {
	 ...
	 */
	public ClinicianLoadPreferencesAction(Connection conn, String clinicianName) throws SQLException {
	 ...
	 * @throws SQLException the SQL exception
	 */
	public ClinicianPreferencesBean loadClinicianPreferences() throws SQLException {
		return clinicianPreferencesDAO.loadClinicianPreferences(clinicianID);
	}
	
	/**
	 * Given a list of commitment beans, groups together all commitment beans referring to the
	 * same commitment. Each sublist is sorted in order of increasing date.
	 *
	 * @return the commitment list
	 * @throws SQLException the SQL exception
	 */
	public List<List<CommitmentBean>> loadCommitments() throws SQLException {
		List<CommitmentBean> commitments = commitmentsDAO.loadCommitments(clinicianID);
	...
	/**
	 * For each commitment list in the 2d list of commitmentbeans, creates a string
	 * which is representative of that commitment and meaningful for display to the user.
	 *
	 * @param commitmentList the commitment list
	 * @return the commitment strings
	 */
	public List<String> loadCommitmentDescriptions(List<List<CommitmentBean>> commitmentList) {
