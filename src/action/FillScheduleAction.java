...
public class FillScheduleAction {
    ...
    /**
     * Instantiates a new fill schedule action.
     *
    ...
    /**
     * Assigns clinicians to all sessions generated by
     * GenerateUnfilledScheduleAction.
     */
	...
	try {
		...
		// for every new week regenerate our 3 arraylists for morning,
		// noon, afternoon clinician ids
		if (currDayOfWeek < lastDayOfWeek) {
		    ecAssignments = generateEcAssignments(allClinicians,
			    clinicianDAO, clinicianPreferencesDAO);
		...
		int index = 0;
		while (extendedClinicians.size() < 16
			&& allClinicians.size() > 0) {
		    extendedClinicians.add(allClinicians.get(
			    index % allClinicians.size()).getClinicianID());
		    index++;
		}
		...
		if (sb.getStartTime() == 11) {
		    sessionsDAO.insertSessionClinicians(sb.getID(),
			    timeslot1.get(dayOfWeek));
		} else if (sb.getStartTime() == 13) {
		    sessionsDAO.insertSessionClinicians(sb.getID(),
			    timeslot2.get(dayOfWeek));
		} else if (sb.getStartTime() == 14) {
		    sessionsDAO.insertSessionClinicians(sb.getID(),
			    timeslot3.get(dayOfWeek));
		} else {
		    sessionsDAO.insertSessionClinicians(sb.getID(),
			    timeslot4.get(dayOfWeek));
		}
	    }
     ...
     */
    private HashMap<Integer, Integer> generateEcAssignments(
	    List<ClinicianBean> allClinicians, ClinicianDAO clinicianDAO,
	    ClinicianPreferencesDAO clinicianPreferencesDAO)
	    throws SQLException {
	List<ClinicianPreferencesBean> clinicians = buildPreferencesList(
		allClinicians, clinicianDAO, clinicianPreferencesDAO);
	HashMap<Integer, Integer> ecAssignments = new HashMap<Integer, Integer>(); // mapping
										   // id
										   // to
										   // assignment
     ...
     * @return ecAssignments
     */
    private HashMap<Integer, Integer> assignECSlotsForE5E5E5(
	    List<ClinicianPreferencesBean> clinicians) {
	HashMap<Integer, Integer> ecAssignments = new HashMap<Integer, Integer>();
	for (ClinicianPreferencesBean pref : clinicians) {
	    ecAssignments.put(pref.getClinicianID(),
		    new Integer(pref.getRanking(1)));
	}
	return ecAssignments;
    }

    /**
     * Assignment algorithm for case where one count < 5, one count == 5, and
     * one count > 5.
     *
     * @param clinicians the clinicians
     * @param prefOneCounts the pref one counts
     * @return ecAssignments
     */
    private HashMap<Integer, Integer> assignECSlotsForLT5E5GT5(
	    List<ClinicianPreferencesBean> clinicians, int[] prefOneCounts) {
	HashMap<Integer, Integer> ecAssignments = new HashMap<Integer, Integer>();
	int indexFor5 = -1;
	int indexForLT5 = -1;
	int indexForGT5 = -1;
	...
	for (int i = 14; i >= 0; i--) {
		...
		ecAssignments.put(new Integer(clinicians.get(i)
			.getClinicianID()), new Integer(clinicians.get(i)
			.getRanking(1)));
	...
	if (clinicians.size() > 0) {
		...
		ecAssignments.put(new Integer(clinicians.get(0)
			.getClinicianID()), new Integer(indexForLT5 + 1));
	    ...
	    for (int i = 0; i < 5; i++) {
		ecAssignments.put(new Integer(clinicians.get(0)
			.getClinicianID()), new Integer(indexForGT5 + 1));
     ...
     * @return ecAssignments
     */
    private HashMap<Integer, Integer> assignECSlotsForLT5GT5GT5(
	    List<ClinicianPreferencesBean> clinicians, int[] prefOneCounts) {
	HashMap<Integer, Integer> ecAssignments = new HashMap<Integer, Integer>();
	...
	for (int i = clinicians.size() - 1; i >= 0; i--) {
	    if (clinicians.get(i).getRanking(1) - 1 == indexForGT5a
		    && clinicians.get(i).getRanking(2) - 1 == indexForLT5) {
		ecAssignments.put(new Integer(clinicians.get(i)
			.getClinicianID()), new Integer(indexForLT5 + 1));
		clinicians.remove(i);
	...
	for (ClinicianPreferencesBean pref : clinicians) {
	    ecAssignments.put(new Integer(pref.getClinicianID()), new Integer(
		    pref.getRanking(1)));
	}
	return ecAssignments;
    }
     ...
     * @return ecAssignments
     */
    private HashMap<Integer, Integer> assignECSlotsForLT5LT5GT5(
	    List<ClinicianPreferencesBean> clinicians, int[] prefOneCounts) {
	HashMap<Integer, Integer> ecAssignments = new HashMap<Integer, Integer>();
	...
	for (int i = clinicians.size() - 1; i >= 0; i--) {
	    if (clinicians.get(i).getRanking(2) - 1 == indexForLT5a
		    && countExcessAssignedLT5a < excessLT5a) {
		ecAssignments.put(new Integer(clinicians.get(i)
			.getClinicianID()), new Integer(indexForLT5a + 1));
		clinicians.remove(i);
		countExcessAssignedLT5a++;
	    } else if (clinicians.get(i).getRanking(2) - 1 == indexForLT5b
		    && countExcessAssignedLT5b < excessLT5b) {
		ecAssignments.put(new Integer(clinicians.get(i)
			.getClinicianID()), new Integer(indexForLT5b + 1));
		clinicians.remove(i);
	...
	for (int i = 0; i < 5; i++) {
	    ecAssignments.put(new Integer(clinicians.get(i).getClinicianID()),
		    new Integer(clinicians.get(i).getRanking(1)));
	}
    ...
    /**
     * Helper method to count the number of clinicians who want each time slot,
     * for their first preference or second preference.
     *
     * @param clinicianPrefs the clinician prefs
     * @param rank the rank
     * @return the int[]
     */
    private int[] countRankings(List<ClinicianPreferencesBean> clinicianPrefs,
	    int rank) {
    ...
    /**
     * Given the list of all clinicians in the system, return a list of 15
     * random clinicians associated with their first and second choice time
     * slots. If there are less than 15 clinicians in the system, duplicate
     * allClinicians and randomly select clinicians to fill any left over spots,
     * to make the list length 15.
     *
     * @param allClinicians the all clinicians
     * @param clincianDAO the clincian dao
     * @param clinicianPreferencesDAO the clinician preferences dao
     * @return the list
     * @throws SQLException the SQL exception
     */
    private List<ClinicianPreferencesBean> buildPreferencesList(
	    List<ClinicianBean> allClinicians, ClinicianDAO clincianDAO,
	    ClinicianPreferencesDAO clinicianPreferencesDAO)
	    throws SQLException {
	...
	if (allClinicians.size() >= 15) {
	    List<ClinicianBean> beans = new ArrayList<ClinicianBean>(
		    allClinicians.subList(0, 15));
	    for (ClinicianBean bean : beans) {
		clinicians.add(clinicianPreferencesDAO
			.loadClinicianPreferences(bean.getClinicianID()));
	    }
	} else {
	    List<ClinicianBean> beans = new ArrayList<ClinicianBean>(
		    allClinicians.subList(0, (int) (15 - allClinicians.size()
			    * Math.floor(15.0 / allClinicians.size()))));
	    for (ClinicianBean bean : beans) {
		clinicians.add(clinicianPreferencesDAO
			.loadClinicianPreferences(bean.getClinicianID()));
	    }
		...
		for (int i = 0; i < Math.floor(15.0 / allClinicians.size()); i++) {
		    ...
		    clinicians.add(clinicianPreferencesDAO
			    .loadClinicianPreferences(cb.getClinicianID()));
		}
	    }
	}
	return clinicians;
