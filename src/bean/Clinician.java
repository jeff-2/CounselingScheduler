...
public class Clinician {
	 ...
	 *            the tab
	 */
	public Clinician(ClinicianBean cb, ClinicianPreferencesBean cbPref,
			List<CommitmentBean> cmb, List<TimeAwayBean> tab) {
		clinicianBean = cb;
	...
	public boolean canCover(SessionBean session) {
		for (CommitmentBean cb : this.commitmentBeans) {
			if (cb.getDate().equals(session.getDate())
					...
					&& cb.getEndHour() >= (session.getDuration() + session
							.getStartTime())) {
				return false;
			}
		}
		if (session.getType() == SessionType.EC) {
			...
			for (TimeAwayBean tb : this.timeAwayBeans) {
				...
				if (!sd.before(tbStart) && !sd.after(tbEnd)) {
					return false;
				}
			}
		}
