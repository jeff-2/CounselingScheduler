...
public class ECProgram {
	...
	try {
		...
		for (String clinician : clinicians) {
		    ...
		    GRBVar var = model.addVar(0.0, 1.0, 1.0, GRB.BINARY,
			    varName);
		...
		for (String week : clinicianWeekVars.get(clinician).keySet()) {
		    GRBLinExpr expr = new GRBLinExpr();
		    for (GRBVar var : clinicianWeekVars.get(clinician)
			    .get(week)) {
			expr.addTerm(1.0, var);
		    }
		    model.addConstr(expr, GRB.LESS_EQUAL, 1.0, clinician + "_"
			    + week + "_lessthan1");
		}
	    }
