...
public class ScheduleProgram {
	...
	for (Clinician clinician : this.clinicianList) {
	    this.clinicianNameLookup.put(
		    clinician.getClinicianBean().getName(), clinician);
	}
	...
	for (SessionBean session : this.sessions) {
	    Week week = Week.getWeek(session.getDate(),
		    this.schedule.getCalendar());
	...
	try {
		...
		for (SessionBean session : this.sessionsByWeek.get(week)) {
			...
			if (clinician.canCover(session)) {
			    GRBVar var;
			    String varName = clinician.getClinicianBean()
				...
				int time = start == 8 ? 1 : (start == 12 ? 2
					: 3);
				double pref = prefs[clinician
					.getClinicianPreferencesBean()
					.getRankingFromTime(time)];
				var = model.addVar(0.0, 1.0, pref, GRB.BINARY,
					varName);
		...
		if (session.getType() == SessionType.EC) {
		    ...
		    model.addConstr(expr, GRB.GREATER_EQUAL,
			    session.getDuration(), session + "_filled");
		    model.addConstr(expr, GRB.LESS_EQUAL,
			    session.getDuration() + 4, session + "_filled");
		}
	    }
	    // Each clinician scheduled for EC no more than once per week, for
	    // IA no more than once per day
	    for (Clinician clinician : this.clinicianList) {
		int ecHours = clinician.getClinicianPreferencesBean()
			.getECHours();
		int iaHours = clinician.getClinicianPreferencesBean()
			.getIAHours();
		...
		for (Week week : this.weeks) {
			...
			for (GRBVar var : clinicianWeekVars.get(clinician).get(
				week)) {
				...
				if (label.contains("_16_")) {
					...
					model.addConstr(pairExpr,
						GRB.LESS_EQUAL, 1.0, clinician
							.getClinicianBean()
							.getName()
							+ "_"
							+ week
				    ...
				    int prevDay = Weekday.getIndexOfDay(label
					    .split("_")[2]);
				    if (prevDay >= 0) {
					for (SessionBean lateSesh : lateSessions
						.get(prevDay)) {
					...
					if (available) {
					    for (SessionBean lateSesh : lateSessions
						    .get(prevDay)) {
						String varName = clinician
							.getClinicianBean()
							.getName()
							+ "_"
							+ lateSesh
								.getVariableString();
						GRBVar ecVar = this.ecVars
							.get(varName);
						if (ecVar != null) {
						    GRBLinExpr pairExpr = new GRBLinExpr();
						    pairExpr.addTerm(1.0, var);
		...
		model.addConstr(ecsPerSemester, GRB.LESS_EQUAL, ecHours,
			clinician + "_ecs_islessthan_" + ecHours);
		model.addConstr(afternoonECs, GRB.GREATER_EQUAL,
			afternoonECmin, clinician
				+ "_afternoon_ecs_ismorethan_" + afternoonECmin);
	    }
	...
	try {
		...
		for (GRBVar var : this.sessionVars.get(session)) {
		    ...
		    if (val > 0.0) {
			Clinician clinician = this.clinicianNameLookup
				.get(label.split("_")[0]);
