...
/**
 * The Class ClinicianPreferencesDao handles interactions with the database to
 * access the ClinicianPreferences table.
 * 
...
public class ClinicianPreferencesDAO extends DAO {
	...
	public ClinicianPreferencesBean loadClinicianPreferences(int clinicianID)
		...
		ClinicianPreferencesBean preferences = new ClinicianPreferencesBean(
	...
	/**
	 * Returns true if there are preferences stored for the specified clinician
	 * id
	 *
	 * @param clinicianID
	 *            the clinician id
	...
	public void delete(int clinicianID) throws SQLException {
		PreparedStatement stmt = connection
				.prepareStatement("DELETE FROM ClinicianPreferences WHERE id = ?");
		stmt.setInt(1, clinicianID);
		stmt.execute();
		stmt.close();
	}
