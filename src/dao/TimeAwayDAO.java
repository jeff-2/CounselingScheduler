...
/**
 * The Class TimeAwayDao handles the interaction with the database for the table
 * TimeAway.
 * 
...
public class TimeAwayDAO extends DAO {
	...
	public List<TimeAwayBean> loadTimeAway(int clinicianID) throws SQLException {
		PreparedStatement stmt = connection
				.prepareStatement("SELECT startDate, endDate, description FROM TimeAway WHERE id = ?");
	 ...
	 *             the SQL exception
	 */
	private TimeAwayBean loadTimeAway(int clinicianID, ResultSet results)
			throws SQLException {
	...
	/**
	 * Deletes all the time away from the database for a particular clinician
	 * id.
	 *
	 * @param clinicianID
	 *            the clinician id
	...
	public void delete(int clinicianID) throws SQLException {
		PreparedStatement stmt = connection
				.prepareStatement("DELETE FROM TimeAway WHERE id = ?");
		stmt.setInt(1, clinicianID);
		stmt.execute();
		stmt.close();
	}
