...
/**
 * Interface for the administrator to initialize the settings for the new
 * semester.
 * 
 * @author jmfoste2
 * @author nbeltr2
...
public class NewSemesterSettings extends JPanel implements ActionListener {
		...
		submitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (createSemester()) {
					...
					try {
						calendar.setStartDate(format.parse(startDateText
								.getText()));
					...
					calendar.setSemester((Semester) semesterSeasonBox
							.getSelectedItem());
					calendar.setEcMinHours(Integer.parseInt(ECHoursText
							.getText()));
					calendar.setIaMinHours(Integer.parseInt(IAHoursText
							.getText()));
					...
					try {
						...
						if (!excelFilenameLabel.getText().equals(
								"No File Selected")) {
							File meetingFile = new File(excelFilenameLabel
									.getText());
							calendar.setMeetingFilepath(meetingFile
									.getAbsolutePath());
							ImportClinicianMeetingsAction action = new ImportClinicianMeetingsAction(
									conn, meetingFile);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == addHolidayButton) {
			...
			if (choice == JFileChooser.APPROVE_OPTION) {
				excelFilenameLabel.setText(fileChooser.getSelectedFile()
						.getAbsolutePath());
	...
	private boolean createSemester() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Cannot create new semester. "
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Cannot create new semester. "
					+ e.getMessage(), "Setting invalid semester settings",
					JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	private void removeHoliday() {
		...
		if (index >= 0) {
			DefaultListModel<String> oldModel = (DefaultListModel<String>) holidayStringList
					.getModel();
	...
	private void addHoliday() throws ParseException {
		...
		if (holiday.isEmpty()) {
			JOptionPane.showMessageDialog(this,
					...
					"Adding invalid holiday", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		try {
			...
			JOptionPane.showMessageDialog(
					...
					"Cannot add the given holiday to the list. "
							+ e.getMessage(), "Adding invalid holiday",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		DefaultListModel<String> model = (DefaultListModel<String>) holidayStringList
				.getModel();
	...
	private void loadSettings() throws SQLException {
		...
		startDateText.setText(DateRangeValidator.formatDate(calendar
				.getStartDate()));
		endDateText
				.setText(DateRangeValidator.formatDate(calendar.getEndDate()));
