...
public class ClinicianIDListEditor extends JPanel implements ActionListener,
    ...
    /**
     * Initializes the Connections object that allows us to communicate with the
     * database.
     */
    ...
    /**
     * Update the JList to reflect the current list of clinicians in the
     * database.
     */
	...
	if (e.getSource() == this.addButton) {
		...
		ClinicianForm form = new ClinicianForm(
			calendarBean.getSemester(), calendarBean.getYear(),
			new DateRange(calendarBean.getStartDate(),
				calendarBean.getEndDate()), true, clinicianList
    ...
    /**
     * If there is no new clinician ID text, gray out add user button. If no
     * selected clinician in list, gray out remove user button.
     */
	...
	try {
	    this.editButton.setEnabled(!noListSelection
		    && prefsDAO.preferencesExist(this.clinicianList
			    .getSelectedValue().getClinicianID()));
    ...
    /**
     * Adds a new clinician ID to the list, where the ID is taken from the input
     * text box. Checks that the new ID is non-empty and that the ID does not
     * already exist. Creates an error g if this is not the case.
     */
	...
	if (newClinicianName.isEmpty()) {
	    JOptionPane
		    .showMessageDialog(
			    ...
			    "Adding empty ID", JOptionPane.ERROR_MESSAGE);
		...
		ClinicianBean newClinician = new ClinicianBean(newClinicianID,
			newClinicianName);
    ...
    /**
     * Removes the selected clinician ID from the list. Checks that a clinician
     * ID is actually selected.
     */
