...
/**
 * The Class ClinicianFormValidator handles the validation of the ClinicianForm
 * input.
 */
public class ClinicianFormValidator {
	 ...
	 *
	 * @param name
	 *            the time away description
	 * @param startDate
	 *            the time away start date
	 * @param endDate
	 ...
	 */
	public static TimeAwayBean validateTimeAway(String name, String startDate,
			String endDate) throws InvalidFormDataException {
		if (name.isEmpty()) {
		...
		try {
			...
			throw new InvalidFormDataException(
					"Cannot add the time away to the list. " + e.getMessage(),
					"Adding invalid time away");
		}
		return new TimeAwayBean(-1, name, dateRange.getStartDate(),
	 ...
	 *
	 * @param dateRange
	 *            the date range for the semester start/end dates
	 * @param description
	 *            the description of the commitment
	 * @param startTime
	 *            the start time of the commitment
	 * @param endTime
	 *            the end time of the commitment
	 * @param dayOfWeek
	 *            the day of week of the commitment
	 * @param frequency
	 *            the frequency of the commitment
	 * @param isExternal
	 *            whether the commitment is external and requires travel
	 * @return the list of commitments generated from the given input
	 * @throws InvalidFormDataException
	 *             the invalid form data exception
	 */
	public static List<CommitmentBean> validateCommitment(DateRange dateRange,
			String description, String startTime, String endTime,
			String dayOfWeek, String frequency, boolean isExternal)
			throws InvalidFormDataException {
		...
		if (startHour >= endHour || sTime.equals(eTime)) {
			throw new InvalidFormDataException(
					...
					"Adding invalid time range for commitment");
		}
		...
		if (frequency.equals("Weekly")) {
			meetingDates = ImportClinicianMeetingsAction.getMeetingDatesWeekly(
					dateRange.getStartDate(), dateRange.getEndDate(), 7,
					dayOfWeek);
		} else if (frequency.equals("Biweekly")) {
			meetingDates = ImportClinicianMeetingsAction.getMeetingDatesWeekly(
					dateRange.getStartDate(), dateRange.getEndDate(), 14,
					dayOfWeek);
		} else {
			meetingDates = ImportClinicianMeetingsAction
					.getMeetingDatesMonthly(dateRange.getStartDate(),
							dateRange.getEndDate(), 1, dayOfWeek);
		}

		List<CommitmentBean> list = new ArrayList<CommitmentBean>();
		for (Date meetingDate : meetingDates) {
			list.add(new CommitmentBean(-1, startHour, endHour, meetingDate,
					description));
		}
		return list;
	}
	 ...
	 *            the clinician name
	 * @param morningRank
	 *            the morning rank preference
	 * @param noonRank
	 *            the noon rank preference
	 * @param afternoonRank
	 *            the afternoon rank preference
	 * @param myIAHours
	 *            the my ia hours preference
	 * @param myECHours
	 *            the my ec hours preference
	 * @param isAdmin
	 *            whether it is an admin (in which case ia/ec hours are
	 *            displayed)
	 ...
	 *             the SQL exception
	 */
	public static ClinicianPreferencesBean validatePreferences(
			String clinicianName, int morningRank, int noonRank,
			int afternoonRank, String myIAHours, String myECHours,
			boolean isAdmin) throws InvalidFormDataException, SQLException {
		...
		if (morningRank == noonRank || afternoonRank == morningRank
				|| noonRank == afternoonRank) {
			throw new InvalidFormDataException(
		...
		if (isAdmin) {
			iaNumHours = Utility.parseInt(myIAHours, "assigned IA hours",
					"Adding clinician ia preferences");
			ecNumHours = Utility.parseInt(myECHours, "assigned EC hours",
					"Adding clinician ec preferences");
		} else {
		...
		return new ClinicianPreferencesBean(clinicianID, morningRank, noonRank,
				afternoonRank, iaNumHours, ecNumHours);
	}
}
